        ...
        <div class="p-6">
            <div class="grid grid-cols-1 gap-4">
                <div class="bg-blue-50 p-4 rounded-lg">
                    ...
                    <p class="text-2xl font-bold text-blue-800">{{ number_format($ticketsSoldToday) }}</p>
                ...
                <div class="bg-green-50 p-4 rounded-lg">
                    ...
                    <p class="text-2xl font-bold text-green-800">Rp {{ number_format($revenueToday, 0, ',', '.') }}</p>
                ...
                <div class="bg-yellow-50 p-4 rounded-lg">
                    ...
                    <p class="text-2xl font-bold text-yellow-800">{{ number_format($activeSchedules) }}</p>
                ...
                <div class="bg-purple-50 p-4 rounded-lg">
                    ...
                    <p class="text-2xl font-bold text-purple-800">{{ $capacityFilled }}%</p>
...
<div class="bg-white rounded-lg shadow">
    ...
    <div class="p-6">
        @if(!empty($weeklySales['days']) && count($weeklySales['days']) > 0)
            <div class="h-72 flex flex-col">
                <!-- Chart Header -->
                <div class="flex justify-between items-center mb-4">
                    <div>
                        <p class="text-sm text-gray-500">Total Pendapatan</p>
                        <p class="text-2xl font-bold text-gray-800">Rp {{ number_format($weeklySales['total'], 0, ',', '.') }}</p>
                    </div>
                    <div class="flex items-center">
                        <span class="text-sm {{ $weeklySales['growth'] >= 0 ? 'text-green-600' : 'text-red-600' }}">
                            {{ $weeklySales['growth'] >= 0 ? '↑' : '↓' }} {{ abs($weeklySales['growth']) }}%
                        </span>
                        <span class="text-sm text-gray-500 ml-1">vs minggu lalu</span>
                    </div>
                </div>

                <!-- Chart Container -->
                <div class="flex-1 flex items-end space-x-1">
                    @foreach($weeklySales['days'] as $day)
                    <div class="flex-1 flex flex-col items-center group relative" style="height: 100%">
                        <!-- Tooltip -->
                        <div class="hidden group-hover:block absolute bottom-full mb-2 px-2 py-1 text-xs text-white bg-gray-900 rounded-md shadow-lg z-10 min-w-[80px] text-center">
                            <div class="font-medium">{{ $day['day'] }}</div>
                            <div>Rp {{ number_format($day['revenue'], 0, ',', '.') }}</div>
                        </div>

                        <!-- Bar Chart -->
                        <div class="w-full h-full flex flex-col justify-end">
                            <!-- Background bar (light gray) -->
                            <div class="w-full bg-gray-100 rounded-t-full" style="height: {{ 100 - $day['percentage'] }}%"></div>
                            <!-- Actual bar -->
                            <div
                                class="w-full bg-gradient-to-t from-blue-600 to-blue-400 rounded-t-full hover:from-blue-700 hover:to-blue-500 transition-all duration-200"
                                style="height: {{ $day['percentage'] }}%"
                            ></div>
                        </div>

                        <!-- Day Label -->
                        <div class="text-xs text-gray-500 mt-1">{{ $day['day'] }}</div>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="grid grid-cols-2 gap-4 mt-6">
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <p class="text-xl font-bold text-gray-800">{{ number_format($weeklySales['tickets']) }}</p>
                    <p class="text-xs {{ $weeklySales['tickets_growth'] >= 0 ? 'text-green-600' : 'text-red-600' }}">
                        {{ $weeklySales['tickets_growth'] >= 0 ? '↑' : '↓' }} {{ abs($weeklySales['tickets_growth']) }}% dari minggu lalu
                    </p>
                </div>
                <div class="bg-gray-50 p-4 rounded-lg">
                    <p class="text-sm font-medium text-gray-500">Rata-rata/Hari</p>
                    <p class="text-xl font-bold text-gray-800">Rp {{ number_format($weeklySales['total']/7, 0, ',', '.') }}</p>
        ...
        @else
            <div class="text-center py-8 text-gray-500">
                ...
                <p class="mt-2">Data penjualan mingguan tidak tersedia</p>
            </div>
        @endif
    </div>
</div>
