  ...
  <div class="max-w-6xl mx-auto text-center">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
      @foreach (['Island Paradise', 'Sunset Bay', 'Adventure Coast'] as $i => $place)
        <div class="bg-blue-50 p-6 rounded-xl shadow-md hover:shadow-xl transition duration-300 opacity-0 translate-y-6 animate-fade-up delay-[{{ $i * 100 }}ms]">
          <img src="https://via.placeholder.com/300x200" class="rounded-lg mb-4 w-full" alt="{{ $place }}">
  ...
  <div class="max-w-xl mx-auto">
    ...
    <form action="#" method="POST" class="space-y-4 bg-white p-6 rounded-xl shadow-md">
      <div>
        ...
        <input id="fullname" name="fullname" type="text" class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring" placeholder="Your name" required>
      </div>
      <div>
        <label class="block mb-1 font-medium" for="destination">Destination</label>
        <select id="destination" name="destination" class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring" required>
      ...
      <div>
        ...
        <input id="travel_date" name="travel_date" type="date" class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring" required>
      </div>
